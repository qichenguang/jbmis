...
class AlertController extends Controller {
    ...
    private $time_alert_array = array(
        ...
        array('lx'=>2,'fieldA' => "ys_cg_nkt_tj_time",'secA'=>0,'fieldB' =>"gc_kg_sj_time",'secB' =>0,'condition'=>'gt',
    ...
    private function find_alert_time($rec){
        ...
        foreach($this->time_alert_array as $cur){
            ...
            if(!empty($a_str) && !empty($b_str)){
                $a_time = strtotime($a_str);
                $b_time = strtotime($b_str);
                $a_time += intval($cur['secA']);
                $b_time += intval($cur['secB']);
                if("gt" == $cur["condition"]){
                    if($a_time > $b_time){
                        $alert_msg = $cur["alert_msg"];
                        str_replace("FIELDA", $a_time, $alert_msg);
                        str_replace("FIELDB", $b_time, $alert_msg);
                        $alert_array = array(
                            'lx' => $cur['lx'],
